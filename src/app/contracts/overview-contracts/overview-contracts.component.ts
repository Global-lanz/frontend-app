import { Component, computed, inject, input, signal, effect, ChangeDetectionStrategy, Signal } from '@angular/core';
...
export class OverviewContractsComponent {
  ...
  private updatePageSizeService = effect(() => {this.contractsService.pageSizeService.set(this.pageSize())});
  private updatePageNumberService = effect(() => {this.contractsService.pageNumberService.set(this.pageNumber())});
  ...
  pagination = this.contractsService.contractsPagination;
  ...
  sortedFilteredContracts = computed(() => {
      ...
      if (!property) return contracts; //if selected Sort Property doesn't exist
  ...
  onCustomerSelectChange() {
    this.pageNumber.set(0); //reset page number to 0 
  }
